...
{
	//template<typename T>
	//static T converttValue(std::string& _value)
	//{
	//	T outValue;
	//	std::istringstream convert(_value);
	//	return convert >> outValue;
	//}
	...
	*/
	...
	/*	Check whether a string.find(...) is valid or not
		...
		#returns
			true if index >= 0
			false if else */
	static bool isValidStringElement(int _elementIndex)
	{
		return (unsigned)_elementIndex != std::string::npos;
	}
	...
	{
		char delim = '=';
		int sep = _line.find_first_of(delim);
	...
	static bool isSplitValue(std::string& _outValue, std::string& _line, char _delimiter = ':')
	{
		if(_line.size() <= 0)
			return false;
	...
	/*	Fetch a subValue from a multi-value attribute without
		...
		-1 if last subvalue
		*/
	static int getValueByIndex(std::string& _outValue, const std::string& _line, int _valueIndex = 0, char _delimiter = ':')
	{
