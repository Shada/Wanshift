...
PS_MRTOutput main(PS_Input input)
{
	...
	output.normal = float4(0, 1, 0, 0);// float4((normal + 1) * 0.5, 0);		//convert normal from (-1 - 1) to (0-1) to fit a texture
	...
	output.glow = output.color * map_ke.Sample(samLinear, input.tex);
