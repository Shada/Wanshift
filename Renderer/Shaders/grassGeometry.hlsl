...
Texture2D biomBlendMap	: register(t10);
Texture2D biomBlendMap2	: register(t11);
...
void generateBlade(float3 rootPos, inout TriangleStream<GS_Output> triStream, const int numSegments)
{
	...
	float segHeight;
	float3 wind = windMap.SampleLevel(samLinear2,frac(p.xz/2550.0),0).rgb * 0.05+float3(0.01,0,0);
	float windCoeff;
	float segDiv;
	float segDivSqrd;
	float widthOffset;
